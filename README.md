...
4. 项目名称填写：`junze-anju`
5. **重要**：上传整个 `dist/` 目录（包含 `functions/` 子目录）
6. 点击 **Deploy site**

> **注意**：每次前端代码修改后需要重新构建并部署。后台数据修改（服务、案例、FAQ 等）无需重新部署，即时生效。
...
3. 数据库名称填写：`junze-anju-db`

#### 5. 初始化数据库（控制台操作）

1. 进入数据库详情页，点击 **Console** 标签页
2. 复制下方完整的 SQL 代码，粘贴到 Console 输入框中
3. 点击 **Execute** 执行 SQL

> 如果 SQL 太长执行失败，可以分批执行：先执行"创建表结构"部分，再逐个执行"插入数据"部分。
...
| 变量名 | 类型 | 值 | 说明 |
|--------|------|-----|------|
| JWT_SECRET | **密钥（Secret）** | 随机字符串（建议至少32位） | JWT 加密密钥，敏感信息 |
| ADMIN_PASSWORD | **密钥（Secret）** | admin123 | 管理员登录密码，敏感信息 |

> **注意**：两个变量都需要勾选 **Secret** 选项，这样在控制台中不会明文显示。需要同时在 **Production** 和 **Preview** 环境中设置。
