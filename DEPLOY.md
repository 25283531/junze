...
本项目推荐使用 **GitHub 自动部署** 方式，代码推送到 GitHub 后自动触发 Cloudflare 构建和部署。
...
5. 点击 **Create** 创建数据库
6. **重要**：记录数据库 ID（格式：`xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`）

#### 1.2 创建 KV 命名空间

KV 命名空间用于存储营业执照等上传的图片。

1. 左侧菜单进入 **Workers & Pages** -> **KV**
2. 点击 **Create namespace**
3. 命名空间名称填写：`junze-anju-kv`
4. 点击 **Create** 创建命名空间
5. **重要**：记录命名空间 ID（格式：`xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`）

### 2. 初始化数据库

1. 进入 D1 数据库详情页
2. 点击 **Console** 标签页
3. 复制下方完整的 SQL 代码，粘贴到 Console 中执行

> **提示**：如果 SQL 太长执行失败，可以分批执行：先执行"创建表结构"部分，再逐个执行"插入数据"部分。
...
确保您的代码已推送到 GitHub 仓库。如果没有，执行以下命令：
...
# 添加远程仓库
git remote add origin https://github.com/您的用户名/junze.git
...
#### 3.2 连接 Cloudflare Pages 到 GitHub

1. 登录 [Cloudflare 控制台](https://dash.cloudflare.com/)
2. 进入 **Workers & Pages** -> **Pages**
3. 点击 **Create a project**
4. 选择 **Connect to Git**（连接到 Git）
5. 选择您的 GitHub 仓库 `junze`
6. 配置构建设置：
   - **Project name**: `junze-anju`
   - **Production branch**: `main`（或您的主分支名）
   - **Build command**: `npm run build`
   - **Build output directory**: `dist`
7. 点击 **Save and Deploy** 开始首次部署

### 4. 配置资源绑定

部署完成后，需要配置 D1 数据库和 KV 命名空间的绑定。
...
直接在 Console 中执行 SQL 更新语句，例如：

```sql
-- 更新服务信息
UPDATE services SET description = '新描述' WHERE slug = 'jia-zheng-fu-wu';
...
A: 请检查：
1. GitHub Actions 或 Cloudflare 构建日志是否显示成功
...
2. 在 D1 Console 中执行：
   ```sql
   UPDATE admin_users SET password_hash = '新的bcrypt哈希' WHERE username = 'admin';
   ```
...
A: 请检查：
1. 查看 Cloudflare 部署日志中的错误信息
2. 确认 `package.json` 中的依赖是否正确
3. 本地运行 `npm run build` 测试是否能成功构建
4. 确认 Node.js 版本 >= 18.0.0
...
1. 安装依赖：`npm install`
2. 配置 `wrangler.toml` 文件中的 D1 数据库 ID 和 KV 命名空间 ID
3. 启动开发服务器：`npm run dev`

> **注意**：本地开发需要配置 Cloudflare API Token 才能访问 D1 和 KV。对于大多数情况，建议直接在 Cloudflare 预览环境测试。
